...
<script>
    ...
    function checkStockValidity(obj) {
        ...
        let adjustment_type = $(obj).closest('tr').find('#adjustment_type').val();

        if (adjustment_type == 'Out' && Number($(obj).val()) > current_qty) {
            $(obj).val(current_qty);
            toastr.warning('Insufficiant stock!');
        }
        totalQuantity();
    }
    ...
    function AddToTable(object)
    {
        ...
        if (is_added == true) {
            $('.stock-table-body').append(`<tr>
                                            ...
                                            <td class="text-center">
                                                <input type="text" name="adjust_qtys[]" class="form-control transfer-quantity input-sm" placeholder="Enter Qty" onkeyup="checkStockValidity(this)">
                                            </td>
